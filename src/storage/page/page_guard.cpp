...
ReadPageGuard::ReadPageGuard(page_id_t page_id, std::shared_ptr<FrameHeader> frame,
    ...
    : page_id_(page_id),
      ...
      disk_scheduler_(std::move(disk_scheduler)) {
        ...
        is_valid_ = true; // since the bufferpool made it
}
...
ReadPageGuard::ReadPageGuard(ReadPageGuard &&that) noexcept {
  // move constructor -> transfer the ownership from that to this
...
auto ReadPageGuard::operator=(ReadPageGuard &&that) noexcept -> ReadPageGuard & {
  ...
  Drop(); // must release the current guard what it owns
...
void ReadPageGuard::Flush() { 
    ...
    BUSTUB_ENSURE(is_valid_, "tried to flush read guard");
...
void ReadPageGuard::Drop() { 
      ...
      // the readPageGuard has the read latch (shared)
      // i need to unlock it before acquire bmp latch
...
auto WritePageGuard::operator=(WritePageGuard &&that) noexcept -> WritePageGuard & { 
  ...
  // Release what this guard currently owns.
  Drop();
